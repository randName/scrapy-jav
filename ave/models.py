from ave import realms, o2m, m2m, db_url
...
class Actress(Article, Base):
    ...
    @classmethod
    def get_or_create(cls, name, update=False, **kwargs):
        a = db.session.query(cls).filter_by(name=name)
        if not a:
            a = article(**kwargs)
            db.session.add(a)
        elif update:
